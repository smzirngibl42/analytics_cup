...
def classify_recipe(row):
    words = [ingredient.strip() for ingredient in row['RecipeIngredientParts'].split(',')]
...
data['RecipeClass'] = data.apply(classify_recipe, axis=1)

data.to_csv('OmnivorVegiVegan.csv', index=False)
