...
def main():
    ...
    if args.decision_trees is not None:
        ...
        #feature_selector = FeatureSelection(dataloader)
        #feature_selector.select_k_best(15)
    ...
    if args.support_vector_machine is not None:
        ...
        #feature_selector = FeatureSelection(dataloader)
        #feature_selector.recursive_feature_elim()
        #feature_selector.select_k_best(10)
        svm.train()
...
class FeatureSelection():
    ...
    def __init__(self,dataloader):
        self.X, self.y, self.feature_names = dataloader.get()
        #print(self.X)
        #print(self.y)
    ...
    def recursive_feature_elim(self):
        ...
        rfe_estimator = SVC(kernel="linear")    # other classifiers !
        rfe_cv = ms.StratifiedShuffleSplit(n_splits=5, test_size=0.2, random_state=42)
...
class Decision_Trees():
    ...
    def __init__(self, dataloader):
        self.train_set, self.target, self.feature_names = dataloader.get()
...
class SVM():
    ...
    def train(self):
        ...
        rand_list = {"clf__kernel":["rbf"],
                     ...
                     "clf__gamma": stats.uniform(0.01, 3)}
        
        
        grid_list = {"clf__kernel":["rbf"],
                     "clf__C": np.array([2.81]),                #np.arange(2, 10, 2)
                     "clf__gamma": np.array([0.56])}           #np.arange(0.1, 1, 0.2
...
class Dataloader():
    ...
    def get(self):
        ...
        train_set = train_set[(np.abs(stats.zscore(train_set["Time"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["PrepTime"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["CookTime"])) < 3)]

        train_set = train_set[(np.abs(stats.zscore(train_set["Calories"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["FatContent"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["SaturatedFatContent"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["CholesterolContent"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["SodiumContent"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["CarbohydrateContent"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["FiberContent"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["SugarContent"])) < 3)]
        train_set = train_set[(np.abs(stats.zscore(train_set["ProteinContent"])) < 3)]

        #train_set = train_set[(np.abs(stats.zscore(train_set["RecipeServings"])) < 3)]
        ...
        train_set['RecipeCategory'] = pd.Categorical(train_set['RecipeCategory'])
        train_set['RecipeCategory'] = train_set['RecipeCategory'].cat.codes

        train_set['HighProtein'] = pd.Categorical(train_set['HighProtein'])
        train_set['HighProtein'] = train_set['HighProtein'].cat.codes

        train_set['LowSugar'] = pd.Categorical(train_set['LowSugar'])
        train_set['LowSugar'] = train_set['LowSugar'].cat.codes

        diet_labels = {'Diet': {'Omnivore': 0, 'Vegetarian': 1, 'Vegan': 2}}
        train_set = train_set.replace(diet_labels)



        # /---------------------- food content wrangling ----------------------/


        # calculate percentage of calories

        train_set['FatCalPerc'] = (train_set["FatContent"] / train_set["Calories"]).fillna(0)
        train_set['SaturatedFatCalPerc'] = (train_set["SaturatedFatContent"] / train_set["Calories"]).fillna(0)
        train_set['CarbohydrateCalPerc'] = (train_set["CarbohydrateContent"] / train_set["Calories"]).fillna(0)
        train_set['FiberCalPerc'] = (train_set["FiberContent"] / train_set["Calories"]).fillna(0)
        train_set['SugarCalPerc'] = (train_set["SugarContent"] / train_set["Calories"]).fillna(0)
        train_set['ProteinCalPerc'] = (train_set["ProteinContent"] / train_set["Calories"]).fillna(0)


        # threshold setting

        train_set['HighCalorieContent'] = train_set['Calories'].apply(lambda x: 1 if x>=400 else 0)

        train_set['LowToMediumProteinContent'] = train_set['ProteinCalPerc'].apply(lambda x: 1 if x<=0.10 else 0)

        train_set['LowFatContent'] = train_set['FatCalPerc'].apply(lambda x: 1 if x<=0.05 else 0)

        train_set['LowSaturatedFatContent'] = train_set['SaturatedFatCalPerc'].apply(lambda x: 1 if x<=0.05 else 0)

        train_set['LowSugarContent'] = train_set['SugarCalPerc'].apply(lambda x: 1 if x<=0.05 else 0)

        train_set['HighFiberContent'] = train_set['FiberCalPerc'].apply(lambda x: 1 if x>0.10 else 0)

        train_set['HighCholesterolContent'] = train_set['CholesterolContent'].apply(lambda x: 1 if x>150 else 0)

        train_set['HighSodiumContent'] = train_set['SodiumContent'].apply(lambda x: 1 if x>500 else 0)


        # invert HighProtein

        train_set['HighProtein'] = train_set['HighProtein'].replace({0:1, 1:0})                 
        train_set = train_set.rename(columns={'HighProtein': 'LowToMediumProtein'})



        # /----------------------  recipe diet classification  ----------------------/

        
        # clean RecipeIngredientParts
        ...
        train_set["RecipeDiet"] = train_set['RecipeIngredientParts'].apply(classify_recipe)


        # clean RecipeIngredientQuantities
        ...
        train_set['TotalTime'] = train_set['CookTime'] + train_set['PrepTime']
        #train_set['CookTimePerc'] = train_set['CookTime'] / train_set['TotalTime']
        #train_set['CookTimePerc'] = train_set['CookTimePerc'].fillna(0)
        #train_set['PrepTimePerc'] = train_set['PrepTime'] / train_set['TotalTime']
        #train_set['PrepTimePerc'] = train_set['PrepTimePerc'].fillna(0)
        train_set['TimeDiff'] = train_set['Time'] - train_set['TotalTime']
        ...
        train_set = train_set.drop('AuthorId',axis=1)
        train_set = train_set.drop('RecipeId',axis=1)

        #train_set = train_set.drop('RecipeIngredientParts',axis=1)
        #train_set = train_set.drop('RecipeIngredientQuantities',axis=1)

        #train_set = train_set.drop("Name",axis=1)
        train_set = train_set.drop("TestSetId",axis=1)
        #train_set = train_set.drop("Rating",axis=1)

        #train_set = train_set.drop('RecipeYield', axis=1)



        # /----------------------  save transformed tain set  ----------------------/


        train_set.to_csv("train_set_transformed.csv",",")


        # /----------------------  feature selection  ----------------------/
        
        # select features
        train_set = train_set[['HighCalories',
                               'LowToMediumProtein',
                               'LowFat',
                               'LowSugar',
                               'HighFiber',
                               'HighCalorieContent',
                               'LowToMediumProteinContent',
                               'LowFatContent',
                               'LowSugarContent',
                               'HighFiberContent',

                               #'HighCholesterolContent',
                               #'HighSodiumContent',
        ...
        # sample
        train_set = train_set.sample(10000)
        ...
        return train_set, target, feature_names
